...
public class DiffController {
    ...
    public DiffResponse diff(@PathVariable("id") String id) {
        Result result = diffService.compareFromId(id);
        return new DiffResponse(result.isEqual(), result.getDifferences());
    }

    @RequestMapping(value = "/v1/diff/{id}/{side:left|right}", method = RequestMethod.POST)
    public void diff(@PathVariable("id") String id, @PathVariable("side") String side,
